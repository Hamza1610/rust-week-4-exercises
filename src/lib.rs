...
impl LegacyTransactionBuilder {
    ...
    pub fn version(mut self, version: i32) -> Self {
        ...
        Self { version: self.version, inputs: self.inputs, outputs: self.outputs, lock_time: self.lock_time }
    }
    ...
    pub fn add_output(mut self, output: TxOutput) -> Self {
        ...
        self.outputs.extend_from_slice(&[TxOutput {
...
pub fn parse_cli_args(args: &[String]) -> Result<CliCommand, BitcoinError> {
    ...
    match args[0].as_str() {
        "send" => {
            if args.len() < 3 {
                return Err(BitcoinError::ParseError("send command requires amount and address arguments".to_string()));
            }
            Ok(CliCommand::Send { amount: args[1].parse::<u64>().map_err(|e| BitcoinError::ParseError(e.to_string()))?, address: args[2].to_string() })
        },
...
impl TryFrom<&[u8]> for LegacyTransaction {
    ...
    fn try_from(data: &[u8]) -> Result<Self, Self::Error> {
        ...
        if data.len() < 16 {
            ...
            let version = i32::from_le_bytes(data[0..4].try_into().expect("4 bytes"));
            let inputs_count = u32::from_le_bytes(data[4..8].try_into().expect("4 bytes"));
            let outputs_count = u32::from_le_bytes(data[8..12].try_into().expect("4 bytes"));
            let lock_time = u32::from_le_bytes(data[12..16].try_into().expect("4 bytes"));
